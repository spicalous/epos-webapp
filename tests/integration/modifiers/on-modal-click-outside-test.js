  ...
  hooks.beforeEach(async function(assert) {
    ...
    await render(hbs`<div class="is-outside"></div>
                     <div class="is-ancestor">
                       <div class="is-element" {{on-modal-click-outside this.assertClicked}}>
                         <div class="is-inside"></div>
                       </div>
                     </div>`);
  });

  test('does not invoke handler when clicking outside and not parent', async function(assert) {
    await click('.is-outside');
    assert.expect(0);
  });

  test('invokes handler when clicking outside but on an ancestor element', async function(assert) {
    await click('.is-ancestor');
    assert.expect(1);
  });
